...
// Returns { subject, innerBody }. wrapHtml is applied by the caller so we can
// inject the agent-specific unsubscribe footer.
...
// Email bodies (without the wrapping HTML). Wrapped per-call so the
// unsubscribe footer can be agent-specific.
...
function getPostAnalysisEmail(agentId, firstName, step) {
  ...
  if (step === 1) {
    ...
    // Replace generic assessment URL with personalized one (in body, before wrap)
