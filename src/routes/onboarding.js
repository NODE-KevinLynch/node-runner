...
function registerOnboardingRoutes(app, db) {
  ...
  app.post("/api/onboarding", async (req, res) => {
    try {
      const {
        first_name, last_name, email, phone, brokerage, region,
      ...
      if (agent) {
        ...
        // Create new agent
        agent_id = "agent_" + first_name.toLowerCase() + "_" + last_name.toLowerCase().replace(/\s+/g, '_');
        await db.prepare(
      ...
      await db.prepare(
        ...
        uuidv4(), agent_id,
        units_2024 || 0, units_2023 || 0, units_2022 || 0,
        gci_2024 || 0, gci_2023 || 0, gci_2022 || 0,
        net_income || 0, has_budget || '', monthly_spend || 0,
        lead_volume || '', lead_source || '', has_crm || '', crm_name || '',
        primary_challenge || '', income_goal || 0, transaction_goal || 0
      );
      ...
      const bottleneck = diagnoseBottleneck({
        gci_2024, units_2024, lead_volume, lead_source,
        has_crm, primary_challenge
      });

      // 6. Create diagnosis record
      const profile = buildProfile({
        gci_2024, gci_2023, gci_2022,
        units_2024, lead_volume, has_crm, primary_challenge
      });
      ...
      await db.prepare(
        ...
        uuidv4(), agent_id, bottleneck, profile,
        JSON.stringify({ lead_volume, lead_source, has_crm, primary_challenge, has_budget })
      );
...
function diagnoseBottleneck({ gci_2024, units_2024, lead_volume, lead_source, has_crm, primary_challenge }) {
...
// ── Profile Builder ─────────────────────────────────────────────────────────
function buildProfile({ gci_2024, gci_2023, gci_2022, units_2024, lead_volume, has_crm, primary_challenge }) {
  const gci = Number(gci_2024) || 0;
  let tier;
  ...
  const trend = Number(gci_2024) > Number(gci_2023) ? 'growing' :
                Number(gci_2024) < Number(gci_2023) ? 'declining' : 'flat';
